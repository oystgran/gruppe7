...
module.exports = (pool) => {
    ...
    try {
      const result = await pool.query(
        `SELECT s.*, g.name, g.car_number, g.nationality
         FROM campingboka.stays s
         JOIN campingboka.guests g ON s.guest_id = g.id
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      const guestResult = await pool.query(
        `INSERT INTO campingboka.guests (name, car_number, nationality)
      ...
      await pool.query(
        `INSERT INTO campingboka.stays (guest_id, spot_id, check_in, check_out, adults, children, electricity, price)
  ...
  router.delete("/:stayId", async (req, res) => {
    ...
    try {
      await pool.query(`DELETE FROM campingboka.stays WHERE id = $1`, [stayId]);

      const checkResult = await pool.query(
        `SELECT 1 FROM campingboka.stays WHERE guest_id = $1 LIMIT 1`,
        [guestId]
      );

      if (checkResult.rowCount === 0) {
        await pool.query(`DELETE FROM campingboka.guests WHERE id = $1`, [
          guestId,
        ]);
  ...
  // GET archive of stays between two dates
